...
// Only allow ID start like below format
// IDXXX
...
// Specific the length words type allow  in title
...
{
    // Allow valid length at 250 words only
    return strlen($desc) <= 250;
}

function isValidDate($date)
{
    // Sepcial handle date where user could type negative number or '0'
    $handle_date = explode('/',trim($date));
    $dd = intval($handle_date[0]);
    $mm = intval($handle_date[1]);
    $yy = intval($handle_date[2]);
    // We ain't live that long till the year of 32767 :)
    if($dd <= 0 || $mm <= 0 || $yy <= 0 || $dd > 31 || $mm > 13 || $yy > 32767) return false;
    return preg_match('/^\d{2}\/\d{2}\/\d{2}$/', $date);
}

// This function input an file and and id that user already typed in
// Read through the file and check if the id is already exists
...
// This dir is for debugging in localhost
//$data_dir = "../assign1/data/jobs";

// Full File path
$data_dir = "../../data/assign1/jobs";
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    $position_id = isset($_POST["positionID"]) ? trim($_POST["positionID"]) : "";
    $title = isset($_POST["title"]) ? trim($_POST["title"]) : "";
    $description = isset($_POST["description"]) ? trim($_POST["description"]) : "";
    $closing_date = isset($_POST["closingDate"]) ? trim($_POST["closingDate"]) : "";
    $position = isset($_POST["position"]) ? $_POST["position"] : "";
    $contract = isset($_POST["contract"]) ? $_POST["contract"] : "";
    $location = isset($_POST["location"]) ? $_POST["location"] : "";
    $application_by = isset($_POST["applicationBy"]) ? $_POST["applicationBy"] : [];
    ...
    $application_method = implode(",", $app_methods); // For example:  "post,email" or "email" or "post" or ""

    // Delete all the special words that for high security
    $description  = str_replace(" ", "", $description);
    $description = preg_replace("#[[:punct:]]#", "", $description);
...
?>
      <p class="links"><a href="index.php"><i class="fas fa-home"></i> Return to home page</a>
                        <a href="postjobform.php"><i class="fas fa-briefcase"></i> Post another job</a></p>
    </section>
  </main>
  <footer>
