...
function displayJob($matched_jobs) {
    if (!empty($matched_jobs)) {
        $jobsFoundLength = count($matched_jobs);
        echo '<div class="job-info"><i class="fas fa-briefcase"></i> Found ' . "<strong>$jobsFoundLength job(s)</strong>". ' matching your search.</div>';
...
function searchResults($file_path, $title, $full_time, $part_time, $post, $email, $on_going, $fixed_term, $on_site, $remote) {
    ...
    if (!is_file($file_path)) {
        echo '<p class="error-message"><i class="fas fa-times-circle"></i> Error: Job file is not found.</p>';
        return;
    }
    ...
    if ($file_content === false || trim($file_content) === '') {
        echo '<p class="error-message"><i class="fas fa-times-circle"></i> Error: Job is empty or unreadable.</p>';
        return;
    }
    ...
    foreach ($file_lines as $job) {
        ...
        $job_application = (!empty($fields[7]) ? trim($fields[7]) : "N/A");
        ...
        if ($has_criteria) {
            ...
            // Sanitize the title (remove all the speical character)
